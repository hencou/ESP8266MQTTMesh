...
#define dbgPrintln(lvl, msg) if (((lvl) & (EMMDBG_LEVEL)) == (lvl)) Serial.println(msg)
...
void ESP8266MQTTMesh::begin() {
    ...
    //dbgPrintln(EMMDBG_MSG, "MQTT Server: " + mqtt_server);
    //dbgPrintln(EMMDBG_MSG, "MQTT Port: " + String(mqtt_port));
    //dbgPrintln(EMMDBG_MSG, "MQTT User: " + mqtt_username ? mqtt_username : "None");
    //dbgPrintln(EMMDBG_MSG, "MQTT PW: " + mqtt_password? mqtt_password : "None");
    //dbgPrintln(EMMDBG_MSG, "MQTT Secure: " + mqtt_secure ? "True" : "False");
    //dbgPrintln(EMMDBG_MSG, "Mesh Secure: " + mesh_secure ? "True" : "False");
    //dbgPrintln(EMMDBG_MSG, "Mesh Port: " + String(mesh_port));
    ...
    if (strlen(mqtt_server) > 0) {
      ...
      if (mqtt_username || mqtt_password) {
        mqttClient.setCredentials(mqtt_password, mqtt_username);
      }
    }
    //</modified by HC>
...
void ESP8266MQTTMesh::connect() {
    ...
    //changed by HC:
    if (standAloneAP || scanning || ! ap_ptr) {
        scan();
    }
    if (scanning) {
...
void ESP8266MQTTMesh::connect_mqtt() {
    ...
    // Attempt to connect
    if (!mqttClient.connected() && strlen(mqtt_server) > 0) {
    ...
    if ((standAloneAP || mqttDisConnect) && !meshConnect) {
      dbgPrintln(EMMDBG_MQTT, "standAloneAP || mqttDisConnect");
        ...
        for (int i = 1; i <= ESP8266_NUM_CLIENTS; i++) {
          if (espClient[i]) {
            send_message(i, topic, msg, msgType);
          }
        }
      }
    } else if (meshConnect) {
...
void ESP8266MQTTMesh::shutdown_AP() {
    if(! AP_ready)
        return;
    //<disabled by HC: this is also done by OnDisconnected function, doing twice causes crash>
    //for (int i = 1; i <= ESP8266_NUM_CLIENTS; i++) {
        // if(espClient[i]) {
...
uint16_t ESP8266MQTTMesh::mqtt_publish(const char *topic, const char *msg, uint8_t msgType)
{
    ...
    //<added by HC>
    dbgPrintln(EMMDBG_MQTT, "topic: " + String(topic));
    dbgPrintln(EMMDBG_MQTT, "qos: " + String(qos));
    dbgPrintln(EMMDBG_MQTT, "retain: " + String(retain));
    dbgPrintln(EMMDBG_MQTT, "msg: " + String(msg));
    //</added by HC>
